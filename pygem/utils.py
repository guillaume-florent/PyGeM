...
def write_bounding_box(parameters, outfile, write_deformed=True):
    ...
    """
    aux_x = np.linspace(0,
                        parameters.lenght_box[0],
                        parameters.n_control_points[0])
    aux_y = np.linspace(0,
                        parameters.lenght_box[1],
                        parameters.n_control_points[1])
    aux_z = np.linspace(0,
                        parameters.lenght_box[2],
                        parameters.n_control_points[2])
    lattice_y_coords, lattice_x_coords, lattice_z_coords = np.meshgrid(aux_y,
                                                                       aux_x,
                                                                       aux_z)
    ...
    if write_deformed:
        ...
        box_points = np.array([lattice_x_coords.ravel(),
    ...
    # step necessary to set the correct order to the box points
    # for vtkStructuredGrid:
    # Data in vtkStructuredGrid are ordered with x increasing fastest,
    # then y, then z
...
def _write_vtk_box(box_points, filename, dimensions):
    ...
    .. warning::
            ...
            you have to slice the lattice because paraview does not 
            visualize them automatically even in the wireframe visualization.
    """
    # setup points and vertices
    points = vtk.vtkPoints()

    for index in range(0, box_points.shape[1]):
        points.InsertNextPoint(box_points[0, index],
                               box_points[1, index],
                               box_points[2, index])
...
def plot_rbf_control_points(parameters, save_fig=False):
    """
    Method to plot the control points of a RBFParameters class.
    It is possible to save the resulting figure.

    :param RBFParameters parameters: parameters of the
...
def write_points_in_vtp(points, outfile='points.vtp', color=None):
    """
    Method that writes a vtp file containing the given points.
    It can be used for any set of 3D points. Useful to visualize control points
    together with mesh points in the same window.

    :param numpy.ndarray points: coordinates of the points.
        The shape has to be (n_points, 3).
    :param string outfile: name of the output file.
        The extension has to be .vtp. Default is 'points.vtp'.
    :param tuple color: tuple defining the RGB color to assign
        to all the points. Default is blue: (0, 0, 255).
